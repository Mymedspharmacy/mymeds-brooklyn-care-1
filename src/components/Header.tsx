...
export const Header = ({ onRefillClick, onAppointmentClick, onTransferClick }: HeaderProps) => {
  ...
  return (
    <>
      {/* Top Bar */}
      <div className="bg-[#57bbb6] text-white py-3 px-4">
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center text-sm">
          <div className="flex items-center mb-2 md:mb-0">
            <span className="text-lg font-semibold">Protect your health, book your consultation now</span>
          </div>
          <div className="flex items-center">
            <Button 
              onClick={onAppointmentClick}
              className="bg-white text-[#376f6b] hover:bg-gray-100 font-semibold px-6 py-2 rounded-lg shadow-md transition-colors"
            >
              BOOK NOW
            </Button>
          </div>
      ...
      <header className="bg-white shadow-lg sticky top-0 z-50 border-b-2 border-[#57bbb6]">
        <div className="container mx-auto px-4 flex items-center justify-between py-4">
          ...
          <div className="flex items-center cursor-pointer" onClick={() => navigate('/') }>
            <img src="/logo.png" alt="My Meds Pharmacy Logo" className="h-24 w-24 sm:h-28 sm:w-28 md:h-36 md:w-36 object-contain transition-all" />
          </div>
          {/* Hamburger for mobile */}
          <button
            className="lg:hidden flex items-center justify-center p-2 rounded-md text-[#57bbb6] hover:bg-[#f5fefd] focus:outline-none focus:ring-2 focus:ring-[#57bbb6] ml-2"
          ...
          <div className="hidden sm:flex gap-2 md:gap-3">
            <button className="flex items-center gap-2 bg-[#376f6b] text-white font-semibold px-5 py-2 rounded-lg shadow hover:bg-[#5EABD6] hover:text-white transition-colors text-base border-2 border-[#376f6b]" onClick={onRefillClick}>
            ...
            <button className="flex items-center gap-2 bg-[#376f6b] text-white font-semibold px-5 py-2 rounded-lg shadow hover:bg-[#5EABD6] hover:text-white transition-colors text-base border-2 border-[#376f6b]" onClick={onTransferClick}>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" />
              </svg>
              Transfer Rx
            </button>
            <button className="flex items-center gap-2 bg-[#376f6b] text-white font-semibold px-5 py-2 rounded-lg shadow hover:bg-[#5EABD6] hover:text-white transition-colors text-base border-2 border-[#376f6b]" onClick={() => {
