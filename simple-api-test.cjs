    ...
    const tests = [
      {
        ...
        test: async () => {
          try {
            ...
            const response = await axios.post(`${this.baseUrl}/api/prescriptions/refill`, form, {
      ...
      },
      {
        ...
        test: async () => {
          try {
            ...
            const response = await axios.post(`${this.baseUrl}/api/prescriptions/transfer`, form, {
              headers: form.getHeaders()
            });
            return response.status === 200 || response.status === 201;
          } catch (error) {
  ...
  async testErrorHandling() {
    const tests = [
      ...
      },
      {
        ...
        test: async () => {
          try {
            const response = await axios.get(`${this.baseUrl}/api/admin/login`);
            return false; // Should not succeed
          } catch (error) {
            return error.response && (error.response.status === 405 || error.response.status === 404);
          }
        }
      }
    ];
