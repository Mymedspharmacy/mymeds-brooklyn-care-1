...
router.post('/admin-reset', async (req, res) => {
  try {
    ...
    if (typeof payload === 'object' && payload !== null && 'userId' in payload) {
      const user = await prisma.user.findUnique({ where: { id: Number((payload as { userId: string | number }).userId) } });
...
router.get('/me', unifiedAdminAuth, async (req, res) => {
  try {
    const user = await prisma.user.findUnique({ 
      where: { id: Number((req as { user: { userId: string | number } }).user.userId) },
      select: {
        id: true,
        email: true,
        name: true,
