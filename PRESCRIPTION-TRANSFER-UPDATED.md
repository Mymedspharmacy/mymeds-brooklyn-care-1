# ✅ Prescription Transfer Form - Updated Requirements

## 🔄 Changes Made

Updated the prescription transfer endpoint to make several fields **optional**.
...
```javascript
{
  ...
  currentPharmacy: "CVS Pharmacy",   // Optional
  notes: "Need urgent transfer",     // Optional
...
- **File**: "Not uploaded"
- **Notes**: "None"
- **To Pharmacy**: "MyMeds Pharmacy" (automatically set)
...
### Database Storage:
Both `Prescription` and `TransferRequest` records are created with:
- Required fields populated
- Optional fields with defaults or "Not provided/specified"
- File name stored if uploaded, null otherwise
- `toPharmacy` automatically set to "MyMeds Pharmacy"
...
### Endpoint:
```
POST http://localhost:4000/api/prescriptions/transfer
Content-Type: multipart/form-data
```

### Minimal Request:
```bash
curl -X POST http://localhost:4000/api/prescriptions/transfer \
  -F "firstName=John" \
  -F "lastName=Doe" \
  -F "phone=555-1234"
```
...
Both refill and transfer forms now have the same flexible requirements!
