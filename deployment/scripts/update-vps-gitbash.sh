...
# MyMeds Pharmacy VPS Update Script for Git Bash
...
# Load VPS settings
if [ -f "../vps-settings.sh" ]; then
    source ../vps-settings.sh
else
    # Default settings
    VPS_USER="root"
...
if [ -d "$VPS_PATH" ]; then
    ...
    echo "✅ Backup created: $BACKUP_PATH/$BackupName.tar.gz"
else
    echo "⚠️  No existing deployment found to backup"
fi
EOF

# Step 4: Upload new files to VPS
echo ""
